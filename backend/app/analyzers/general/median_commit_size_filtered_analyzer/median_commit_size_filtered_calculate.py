...
async def median_commit_size_filtered_calculate(
    commit_size_samples: list[dict[str, Any]],
    outlier_multiplier: float,
) -> int:
    """
    Calculates the median commit size while ignoring extreme outlier commits.
    ...
    Example:
        ...
        will be ignored when recalculating the filtered median.
    """
    ...
    if not filtered_commit_sizes:
        return round(median(commit_sizes))

    filtered_median = median(filtered_commit_sizes)

    return round(filtered_median)